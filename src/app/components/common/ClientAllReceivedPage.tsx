...
const ClientAllReceivedPage = ({ inputData }: TableComponentProps) => {
    const columns: Column<Input>[] = useMemo(
        () => [
            {
                Header: 'ID',
                accessor: 'id',
                ...
                Cell: ({ row  }) => (
                    <>
                        {row.original.items.map((item: string, index: number) => (
                            <p className={`my-2 ${row.original.delete? "bg-black/50 text-white" : "bg-sky-400/50 "} rounded-xl whitespace-nowrap text-center p-2`}
                               key={index}>{item}</p>))}
                    </>
                ),
            },
    ...
    const HandleDeleteData = (id:number,items:string[],qtn:number[])=>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                HandleRemoveOrder(data?.data??[],inputData,id,items,qtn,'add')
                setPending(true);
    ...
    return (
        <PageCard>
            <div className="flex flex-wrap justify-between items-center w-full h-24">
                <h2 className="card-title whitespace-nowrap">All Received : </h2>
                    <input
                        className={
            ...
            <div className="w-full overflow-x-auto rounded-xl">
                <table {...getTableProps()} className="table min-w-[730px]">
                    <thead>
                    {headerGroups.map((headerGroup: HeaderGroup<Input>) => (
                    ...
                    {isMutating?<LoadingSpin className={'w-8 h-8'}/>:page.map((row) => {
                        ...
                        return (
                            <tr
                                {...row.getRowProps()}
                                className={`${row.original.oop == "مرتجع" && !row.original.delete ? 'badge-warning' : row.original.delete ?"bg-error":"bg-gray-300"}`}
                            >
                                {row.cells.map((cell) => (
                                    <td
                                        {...cell.getCellProps()}
...
export default ClientAllReceivedPage;